...
- [文档] Issue #777 Web UI 语言切换使用仓内自建 `UiLanguageContext` + `uiText` 方案（未引入 `react-i18next` / `i18next` 依赖），仅涉及独立 `dsa.uiLanguage` 持久化与界面文案国际化；不改写 `provider/model/base_url` 运行时配置保存与清理语义。回归依据见 `apps/dsa-web/src/contexts/__tests__/UiLanguageContext.test.tsx`（Storage 读写降级）与 `apps/dsa-web/src/hooks/__tests__/useSystemConfig.test.tsx`（`LITELLM_MODEL`、`OPENAI_BASE_URL`、`OPENAI_VISION_MODEL` 在保存链路保持透传、未迁移）。
- [文档] 本次 UI 改动新增可视化验收指引，要求 PR 附上 `smoke-login-page-zh`、`smoke-home-page-zh`、`smoke-home-page-en`、`smoke-settings-page-zh`、`smoke-settings-page-en`、`smoke-mobile-shell-nav` 截图；若环境不可拍摄页面，请在 PR 说明中给出替代可视证据路径与原因。
- [测试] 本轮 Web 方向验证执行：`cd apps/dsa-web && npm run lint`、`cd apps/dsa-web && npm run build`、`cd apps/dsa-web && npm run test -- src/App.test.tsx src/contexts/__tests__/UiLanguageContext.test.tsx src/hooks/__tests__/useSystemConfig.test.tsx`、`cd apps/dsa-web && npm run test:smoke -- e2e/smoke.spec.ts e2e/report-markdown.spec.ts`（命令已执行；`e2e/smoke.spec.ts` 与 `e2e/report-markdown.spec.ts` 共 12 条用例，全部 skip，原因是未设置 `DSA_WEB_SMOKE_PASSWORD`；其中 `language switch updates UI copy and persists after page refresh` 用例用于语言切换验收）。
