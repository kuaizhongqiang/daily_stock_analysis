...
class StockAnalysisPipeline:
    ...
    def run(
            ...
            for idx, future in enumerate(as_completed(future_to_code)):
                ...
                try:
                    ...
                    if result and result.success:
                        ...
                        if single_stock_notify and not dry_run:
                            logger.info("单股推送已禁用（Agent 模式）：%s", code)
        ...
        if results and not dry_run:
            logger.info("通知推送已禁用（Agent 模式）：%d 条分析结果", len(results))
    ...
    def _send_single_stock_notification(
        ...
        logger.info("单股推送已禁用（Agent 模式）：%s", stock_code)
    ...
    def _save_local_report(
        ...
        try:
            ...
            filepath = os.path.join(report_dir, f"analysis_report_{timestamp}.md")
    ...
    def _send_notifications(
        ...
        """通知推送功能已移除（由 AI Agent 接管）"""
        logger.info("通知推送已禁用（Agent 模式）：%d 条分析结果", len(results) if results else 0)
    ...
    def _generate_aggregate_report(
        ...
        """生成汇总报告的纯文本/Markdown 形式（通知功能已移除，仅作本地保存用）"""
        lines = [
            f"# 股票分析报告",
        ...
        for r in results:
            ...
            name = getattr(r, "name", "")
            advice = getattr(r, "operation_advice", "N/A")
            score = getattr(r, "sentiment_score", "N/A")
            lines.append(f"## {name} ({code})")
            lines.append(f"- 操作建议: {advice}")
            lines.append(f"- 综合评分: {score}")
            decision = getattr(r, "decision_signal", None)
            if decision is not None:
                direction = getattr(decision, "direction", "N/A")
                confidence = getattr(decision, "confidence", "N/A")
                lines.append(f"- 决策方向: {direction}")
